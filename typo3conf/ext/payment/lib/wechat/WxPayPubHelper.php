...
class Common_util_pub
{
	...
	public function getSign($Obj)
	{
		...
		//签名步骤二：在string后加入KEY
		$String = $String."&key=".$GLOBALS['WX_KEY'];
	...
	function postXmlSSLCurl($xml,$url,$second=30)
	{
		...
		curl_setopt($ch,CURLOPT_SSLCERT, $GLOBALS['WX_SSLCERT_PATH']);
		//默认格式为PEM，可以注释
		curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
		curl_setopt($ch,CURLOPT_SSLKEY, $GLOBALS['WX_SSLKEY_PATH']);
		//post提交方式
		curl_setopt($ch,CURLOPT_POST, true);
		curl_setopt($ch,CURLOPT_POSTFIELDS,$xml);
		$data = curl_exec($ch);
...
class Wxpay_client_pub extends Common_util_pub 
{
	...
	function createXml()
	{
	   	$this->parameters["appid"] = $GLOBALS['WX_APPID'];//公众账号ID
	   	$this->parameters["mch_id"] = $GLOBALS['WX_MCHID'];//商户号
...
{	
	function __construct() 
	{
		...
		//设置curl超时时间
		$this->curl_timeout = $GLOBALS['WX_CURL_TIMEOUT'];
	}
...
class NativeCall_pub extends Wxpay_server_pub
{
	...
	function createXml()
	{
		if($this->returnParameters["return_code"] == "SUCCESS"){
		   	$this->returnParameters["appid"] = $GLOBALS['WX_APPID'];//公众账号ID
		   	$this->returnParameters["mch_id"] = $GLOBALS['WX_MCHID'];//商户号
...
class JsApi_pub extends Common_util_pub
{
	...
	{
		$urlObj["appid"] = $GLOBALS['WX_APPID'];
		$urlObj["secret"] = $GLOBALS['WX_APPSECRET'];
	...
	{
		$jsApiObj["appId"] = $GLOBALS['WX_APPID'];
		$timeStamp = time();
