...
class tx_datamintsfeuser_utils {
	...
	public static function getFeUsersTca($feUsersTca) {
		...
		if ($feUsersTca) {
			$globalFeUsersTca['columns'] = \TYPO3\CMS\Core\Utility\ArrayUtility::mergeRecursiveWithOverrule($globalFeUsersTca['columns'], \TYPO3\CMS\Core\Utility\GeneralUtility::removeDotsFromTS($feUsersTca));
		}
	...
	public static function generatePassword($password, $arrGenerate = array()) {
		...
		if (\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::isLoaded('md5passwords')) {
		...
		if (\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::isLoaded('t3sec_saltedpw')) {
			...
			if (tx_t3secsaltedpw_div::isUsageEnabled()) {
				...
				$tx_t3secsaltedpw_phpass = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('tx_t3secsaltedpw_phpass');
	...
	public static function checkPassword($submittedPassword, $originalPassword) {
		...
		else if (\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::isLoaded('md5passwords')) {
			$arrConf = unserialize($GLOBALS['TYPO3_CONF_VARS']['EXT']['extConf']['md5passwords']);
		...
		else if (\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::isLoaded('t3sec_saltedpw')) {
			require_once \TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extPath('t3sec_saltedpw') . 'res/staticlib/class.tx_t3secsaltedpw_div.php';
	...
	public static function userRedirect($pageId = 0, $urlParameters = array(), $disableAccessCheck = FALSE) {
		...
		header('Location: ' . \TYPO3\CMS\Core\Utility\GeneralUtility::locationHeaderUrl($pageLink));
		exit;
	}
...
//		}
	...
	public static function setFlexformConfigurationValue($key, $value, $conf) {
		if (strpos($key, '.') !== FALSE && $value) {
			$arrKey = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode('.', $key, TRUE);
			...
			$conf = \TYPO3\CMS\Core\Utility\ArrayUtility::mergeRecursiveWithOverrule($conf, $value);
		} else if ($value) {
			$conf[$key] = $value;
		}
