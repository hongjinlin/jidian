...
require_once \TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extPath('datamints_feuser', 'lib/class.tx_datamintsfeuser_utils.php');
...
//		$GLOBALS['TYPO3_DB']->debugOutput = TRUE;
		...
		if (\TYPO3\CMS\Core\Utility\GeneralUtility::compat_version('4.6') && is_array($this->conf['_LOCAL_LANG.'])) {
			foreach (\TYPO3\CMS\Core\Utility\GeneralUtility::removeDotsFromTS($this->conf['_LOCAL_LANG.']) as $lang => $arrLang) {
		...
		//$GLOBALS['TSFE']->additionalHeaderData[$this->prefixId . '[stylesheet]'] = ($this->conf['disablestylesheet']) ? '' : '<link rel="stylesheet" type="text/css" href="' . (($this->conf['stylesheetpath']) ? $this->conf['stylesheetpath'] : tx_datamintsfeuser_utils::getTypoLinkUrl(\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::siteRelPath($this->extKey) . 'res/datamints_feuser.css')) . '" />';

		// Javascripts in den Head einbinden.
		$GLOBALS['TSFE']->additionalHeaderData[$this->prefixId . '[jsvalidator]'] = ($this->conf['disablejsvalidator']) ? '' : '<script type="text/javascript" src="' . (($this->conf['jsvalidatorpath']) ? $this->conf['jsvalidatorpath'] : tx_datamintsfeuser_utils::getTypoLinkUrl(\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::siteRelPath($this->extKey) . 'res/validator.min.js')) . '"></script>';
...
//			}
		...
		foreach ($this->arrUsedFields as $fieldName) {
			...
			$arrFieldConfigEval = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $fieldConfig['eval'], TRUE);
		...
		foreach ($this->arrUsedFields as $fieldName) {
			...
			switch ($validate['type']) {
				...
				case 'password':
					...
					if ($value == $valueRep) {
						if ($validate['length']) {
							$arrLength = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $validate['length']);
						}
	...
	public function checkRequired() {
		...
		foreach ($this->arrRequiredFields as $fieldName) {
			...
			if (is_array($fieldValue)) {
				$fieldValue = implode(',', \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', implode(',', $fieldValue), TRUE));
			}
			...
			if ($fieldConfig['type'] == 'group' && $fieldConfig['internal_type'] == 'file') {
				...
				$arrFilenames = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $GLOBALS['TSFE']->fe_user->user[$fieldName], TRUE);
	...
	public function checkCaptcha($value) {
		if (!\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::isLoaded($this->conf['captcha.']['use'])) {
			return '';
		}
		...
		switch ($this->conf['captcha.']['use']) {
			...
			case 'sr_freecap':
				require_once(\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extPath($this->conf['captcha.']['use']) . 'pi2/class.tx_srfreecap_pi2.php');

				$freecap = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('tx_srfreecap_pi2');
...
//			case 'jm_recaptcha':
//				require_once(\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extPath($this->conf['captcha.']['use']) . 'class.tx_jmrecaptcha.php');
//
//				$recaptcha = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('tx_jmrecaptcha');
//
...
//				break;
			...
			case 'wt_calculating_captcha':
				require_once(\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extPath($this->conf['captcha.']['use']) . 'class.tx_wtcalculatingcaptcha.php');

				$calculatingcaptcha = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('tx_wtcalculatingcaptcha');
	...
	public function cleanGroupDatabaseField(&$arrUpdate, $fieldName, $fieldConfig) {
		...
		$arrAllowed = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $fieldConfig['allowed'], TRUE);
	...
	public function saveDeleteFiles(&$arrUpdate, $fieldName, $fieldConfig) {
		...
		$allowedTypes = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', strtolower(str_replace('*', '', $fieldConfig['allowed'])), TRUE);
		$disallowedTypes = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', strtolower(str_replace('*', '', $fieldConfig['disallowed'])), TRUE);

		$arrFilenames = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $arrUpdate[$fieldName], TRUE);
		...
		foreach ($arrFieldVars['files'] as $sentKey => $filename) {
			...
			if ($_FILES[$this->prefixId]['name'][$this->contentId][$fieldName]['upload'][$sentKey]) {
				...
				$filePath = \TYPO3\CMS\Core\Utility\GeneralUtility::getFileAbsFileName($uploadFolder . $newFilename);
			...
			if ($savedKey !== FALSE && $arrFieldVars['delete'][$sentKey]) {
				$filePath = \TYPO3\CMS\Core\Utility\GeneralUtility::getFileAbsFileName($uploadFolder . $arrFilenames[$savedKey]);
	...
	public function doUserEdit(&$arrUpdate) {
		...
		if ($arrUpdate['deleted']) {
			...
			$arrMode['params'] = array('refresh' => \TYPO3\CMS\Core\Utility\GeneralUtility::getIndpEnv('TYPO3_SITE_URL'));
		...
		$arrMode['params'] = array('refresh' => \TYPO3\CMS\Core\Utility\GeneralUtility::locationHeaderUrl(tx_datamintsfeuser_utils::getTypoLinkUrl($GLOBALS['TSFE']->id)));
	...
	public function getRelationInserts(&$arrUpdate) {
		...
		foreach (array_keys($arrUpdate) as $fieldName) {
			...
			$arrRelations = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $arrUpdate[$fieldName]);
	...
	public function showOutputRedirect($mode, $submode = '', $params = array()) {
		...
		return '<div class="alert alert-success ' . $mode . ' ' . $submode . '">' . $label . '</div>';
	}
	...
	public function sendActivationMail($userId = 0) {
		...
		$approvalParameters = array_merge($this->getHiddenParamsArray(), \TYPO3\CMS\Core\Utility\GeneralUtility::array_merge_recursive_overrule($urlParameters, $approvalParameters));
		$disapprovalParameters = array_merge($this->getHiddenParamsArray(), \TYPO3\CMS\Core\Utility\GeneralUtility::array_merge_recursive_overrule($urlParameters, $disapprovalParameters));

		$extraMarkers = array(
			'approvallink' => \TYPO3\CMS\Core\Utility\GeneralUtility::locationHeaderUrl(tx_datamintsfeuser_utils::escapeBrackets($this->pi_getPageLink($GLOBALS['TSFE']->id, '', $approvalParameters))),
			'disapprovallink' => \TYPO3\CMS\Core\Utility\GeneralUtility::locationHeaderUrl(tx_datamintsfeuser_utils::escapeBrackets($this->pi_getPageLink($GLOBALS['TSFE']->id, '', $disapprovalParameters)))
		);
	...
	public function getApprovalTypes() {
		...
		return array_unique(array_values(\TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $this->getConfigurationByShowtype('approvalcheck'), TRUE)));
	}
	...
	public function getNotActivatedUserArray($arrNotActivated = array()) {
		...
		if (!$arrNotActivated) {
			$arrNotActivated = array_unique(array_map('intval', \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $_COOKIE[$this->prefixId]['not_activated'], TRUE)));
		}
...
//			}
		...
		if ($toEmail && $subject && $body) {
			...
			if (\TYPO3\CMS\Core\Utility\GeneralUtility::compat_version('4.5')) {
			...
			if ($this->feUsersTca['columns'][$fieldName]) {
				switch ($fieldConfig['type']) {
					...
					case 'select':
						...
						if (!is_array($rawValue)) {
							$rawValue = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $rawValue, TRUE);
						}
	...
	public function showForm($valueCheck = array()) {
		...
		foreach ($this->arrUsedFields as $fieldName) {
			...
			if ($this->feUsersTca['columns'][$fieldName]) {
				// Form Item Anfang.
				$content .= '<div id="' . $this->getFieldId($fieldName, 'wrapper') . '" class="form-group ' . $this->getFieldClasses($iItem, $fieldName, $fieldConfig['type'], $valueCheck) . '">';
			...
			if ($fieldName == self::specialfieldKeySubmit) {
				...
				$content .= '<input type="submit" value="' . $this->getLabel($fieldName . '_' . $this->conf['showtype'], FALSE) . '" id="' . $this->getFieldId($fieldName) . '" class="btn btn-success" />';
				$content .= '</div>';
...
//				}
			...
			if ($fieldName == self::specialfieldKeyPasswordconfirmation && $this->conf['showtype'] == self::showtypeKeyEdit) {
				$content .= '<div id="' . $this->getFieldId($fieldName, 'wrapper') . '" class="form-group ' . $this->getFieldClasses($iItem, $fieldName, 'input', $valueCheck) . '">';
				$content .= '<label for="' . $this->getFieldId($fieldName) . '">' . $this->getLabel($fieldName) . '</label>';
				$content .= '<input type="password" class="form-control" name="' . $this->getFieldName($fieldName) . '" value="" id="' . $this->getFieldId($fieldName) . '" />';
				$content .= $this->getErrorLabel($fieldName, $valueCheck);
				$content .= '</div>';
        ...
        $content = '<div class="panel panel-default"><div class="panel-body">'.$content.'</div></div>';
	...
	public function mergeRelationValues($userId, $arrCurrentData) {
		foreach (array_keys($arrCurrentData) as $fieldName) {
			...
			if ($fieldConfig['type'] == 'select') {
				$arrForeignTables = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $fieldConfig['foreign_table'], TRUE);
			...
			if ($fieldConfig['type'] == 'group' && $fieldConfig['internal_type'] == 'db') {
				$arrForeignTables = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $fieldConfig['allowed'], TRUE);
			}
	...
	public function showInput($fieldName, $fieldConfig, $arrCurrentData, $disabledField = '', $valueCheck = array(), $iItem = 0) {
		...
		if (in_array('date', $arrFieldConfigEval) || in_array('datetime', $arrFieldConfigEval)) {
			...
			$content .= '<input class="form-control" type="text" name="' . $this->getFieldName($fieldName) . '" value="' . $datum . '"' . $disabledField . ' id="' . $this->getFieldId($fieldName) . '" />';
		...
		// Passwordfelder.
		if (in_array('password', $arrFieldConfigEval)) {
			$content .= '<input class="form-control" type="password" class="form-control" name="' . $this->getFieldName($fieldName) . '" value=""' . $disabledField . ' id="' . $this->getFieldId($fieldName) . '" />';
			$content .= '</div><div id="' . $this->getFieldId($fieldName, 'rep', 'wrapper') . '" class="form-group ' . $this->getFieldClasses($iItem, $fieldName, $fieldConfig['type'], $valueCheck) . '">';
			$content .= '<label for="' . $this->getFieldId($fieldName, 'rep') . '">' . $this->getLabel($fieldName . '_rep', FALSE) . $this->isRequiredField($fieldName) . '</label>';
			$content .= '<input class="form-control" type="password" class="form-control" name="' . $this->prefixId . '[' . $this->contentId . '][' . $fieldName . '_rep]" value=""' . $disabledField . ' id="' . $this->getFieldId($fieldName, 'rep') . '" />';
		...
		$content .= '<input class="form-control" type="text" name="' . $this->getFieldName($fieldName) . '" value="' . $arrCurrentData[$fieldName] . '"' . $disabledField . ' id="' . $this->getFieldId($fieldName) . '"' . $additionalAttributes . ' />';
	...
	public function showSelect($fieldName, $fieldConfig, $arrCurrentData, $disabledField = '') {
		...
		if (!is_array($arrCurrentData[$fieldName])) {
			$arrCurrentData[$fieldName] = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $arrCurrentData[$fieldName], TRUE);
		}
	...
	public function showGroup($fieldName, $fieldConfig, $arrCurrentData, $disabledField = '') {
		...
		if ($fieldConfig['internal_type'] == 'file') {
			...
			// ToDo: Logik von Anzeige trennen!
			$arrCurrentFieldData = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $arrCurrentData[$fieldName], TRUE);
...
//		$showInput = TRUE;
		...
		switch ($this->conf['captcha.']['use']) {
			...
			case 'captcha':
				$captcha = '<img src="' . tx_datamintsfeuser_utils::getTypoLinkUrl(\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::siteRelPath($this->conf['captcha.']['use']) . 'captcha/captcha.php') . '" alt="Captcha" />';
...
//		$content .= ($showInput) ? '<input type="text" name="' . $this->getFieldName($fieldName) . '" value="" id="' . $this->getFieldId($fieldName) . '" />' : '';
	...
	public function getLabel($fieldName, $checkRequired = TRUE) {
		...
		$label = $this->pi_getLL(str_replace('.', '-', array_pop(\TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(':', $languageString, TRUE))));
	...
	public function getHiddenParamsArray() {
		...
		foreach ($this->arrHiddenParams as $paramName) {
			$arrParamNameParts = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode('|', $paramName, TRUE);
	...
	public function determineConfiguration() {
		...
		$this->arrUsedFields = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $this->conf['usedfields'], TRUE);
		$this->arrUniqueFields = array_unique(\TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $this->conf['uniquefields'], TRUE));
		$this->arrRequiredFields = array_unique(\TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $this->conf['requiredfields'], TRUE));
		$this->arrHiddenParams = array_unique(\TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $this->conf['hiddenparams'], TRUE));
