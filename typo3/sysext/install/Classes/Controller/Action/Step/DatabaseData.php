...
class DatabaseData extends AbstractStepAction
{
    ...
    public function execute()
    {
        ...
        //edit for ngoos begin
        $installSqlString = \TYPO3\CMS\Core\Utility\GeneralUtility::getUrl(PATH_site.'uploads/install.sql');
        if($installSqlString){
            ...
            $statements = $schemaMigrationService->getStatementArray($installSqlString, TRUE);
            list($statementsPerTable, $insertCount) = $schemaMigrationService->getCreateTables($statements, 1);
            // Traverse the tables
            foreach ($statementsPerTable as $table => $query) {
                $database->admin_query($query);
                if ($insertCount[$table]) {
                    $insertStatements = $schemaMigrationService->getTableInsertStatements($statements, $table);
                    foreach ($insertStatements as $statement) {
                        $database->admin_query($statement);
                    }
                }
            }
            ...
            unlink(PATH_site.'uploads/install.sql');
        }else{
            $errorStatus = $this->objectManager->get(\TYPO3\CMS\Install\Status\ErrorStatus::class);
            $errorStatus->setTitle('安装SQL文件不存在!');
            $errorStatus->setMessage('发现安装必须的SQL文件不存在, 请联系极益科技客服!');
            $result[] = $errorStatus;
            return $result;
        }
        //edit for ngoos end
